...
import { actionLabel, entityLabel, toneFor } from "../../utils/auditLabels";
...
const AuditList = ({ events, shopsById }) => (
  <ul className="space-y-3 text-gray-800 dark:text-gray-100">
    {events.map((event) => {
      // Colour carries no information the label does not — it only makes the
      // label findable while scrolling. Both are rendered, always, so the log
      // reads the same to someone who cannot distinguish red from green.
      const tone = toneFor(event.action);

      return (
        <li
          key={event.id}
          className={`border border-l-4 ${tone.border} border-gray-200 dark:border-gray-700 rounded-lg p-3 sm:p-4 bg-white dark:bg-gray-800`}
        >
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
            {/* A deleted account still names itself here: the snapshot columns
                are plain text, and audit_log.actor_user_id has no foreign key
          ...
          <AuditChangeSummary changes={event.changes} shopsById={shopsById} />
        </li>
      );
    })}
  </ul>
