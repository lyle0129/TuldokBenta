...
- [x] 1. Add `audit_log` to `initDB.js`
...
- [x] 2. Create `backend/utils/audit.js`
...
- [x] 3. Write `backend/utils/audit.test.js`
...
- [x] 4. Add audit calls to the sale controllers
...
- [x] 5. Add audit calls to the catalog controllers
...
- [x] 6. Add audit calls to `authController.js`
...
- [x] 7. Create `backend/routes/admin.js` and `controllers/auditController.js`
...
- [~] 11. Run the integration checks — **all but the last row done**
  - Every row of the design's integration table ✅, run against scratch shops 2 and 3 on the
    live database; shop 1 was never written to and ended with 6336 closed sales and 0 audit rows
  ...
  - Confirm a manager gets 403 and an unauthenticated caller gets 401 even with
    ...
    legacy actor with 200 while `/api/admin/audit` still answered 401)
  - _Requirements: 4.2, 5.1, 5.2, 5.3_

- [ ] 12. Performance sanity check — **NOT DONE**
  - Generate ~50k synthetic rows across a few months — deferred for the same reason as above:
    the only configured database is production
  ...
  - Page through 250 rows at `limit=100` and confirm three pages with no duplicates and no gaps
    — the keyset walk itself was verified at a smaller scale (41 rows over 14 pages at
    `limit=3`: every row exactly once, no duplicates, no gaps, descending). What is unverified
    is the *plan* at volume, which is the part that actually matters here
  - _Requirements: 1.6, 5.3, 5.4_

- [x] 13. Verification checkpoint
  - `npm test` in `backend/` passes ✅ — 143 tests, 0 failures, stable over repeated runs
  - No response body shape changed on any pre-existing endpoint ✅ — every `recordAudit` call
    sits after its `res.json(...)`, so no handler's response could change even in principle
  - No `changes` payload anywhere contains a password, hash or token ✅ — property P2, plus
    `SELECT COUNT(*) FROM audit_log WHERE changes::text ~* '(password|token|hash)'` = 0
  - The two-shop scoping matrix from ticket 04 still passes unchanged ✅
  - Ask the user if any questions arise before closing out ✅ — the live-verification scope was
    agreed before anything was written to the production database

## Carried into ticket 06

- The two checks left open above (the audit-insert failure end-to-end, and the plan at volume)
